...
export default function SocialIcons({ vertical, className }: Props) {
  return (
    <div className={`flex ${vertical ? "flex-col" : "flex-row"} gap-3 ${className || ""}`}>
      {links.map(({ label, href, svg, icon: Icon }) => (
        <a
          key={label}
          href={href}
          target="_blank"
        ...
        >
          {Icon ? (
            ...
            <svg viewBox="0 0 24 24" width={20} height={20} fill="currentColor" className="text-cream">
              {svg}
            </svg>
          )}
        </a>
